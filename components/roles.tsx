...
const Roles = ({roles}) => {
    return ( 
        <div className='lg:grid grid-cols-2 gap-0 ml-11 lg:mr-44 mr-11 lg:h-screen'>
            <div>
            <h3 className="text-4xl text-center mt-11 mb-11 sticky">Roles</h3>
            </div>
            <div className='grid grid-rows-8 gap-4 lg:overflow-y-auto snap-y'>
               {roles.map((role) => (
                    <a href={role.companyWebsite ?? role.link} target="__blank" key={role.companyName ?? role.clubName}>
                        <div  className='snap-center transition duration-500 hover:-transition container rounded-md p-5 hover:bg-yellow-400 group bg-transparent group-hover:border-5 group-hover:border-yellow-400'>
